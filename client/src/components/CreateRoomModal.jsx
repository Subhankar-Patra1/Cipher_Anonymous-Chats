        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in">
            <div className="bg-slate-900 p-6 rounded-2xl w-full max-w-sm border border-slate-800 shadow-2xl transform transition-all scale-100">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    {type === 'group' ? (
                        <div>
                            <label className="block text-slate-400 text-xs font-bold uppercase tracking-wider mb-1.5">Room Name</label>
                            <input 
                                type="text" 
                                className="w-full bg-slate-800 text-white rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-violet-500/50 border border-slate-700 focus:border-violet-500/50 transition-all placeholder:text-slate-600"
                                value={name}
                        ...
                        <div>
                            <label className="block text-slate-400 text-xs font-bold uppercase tracking-wider mb-1.5">Search User</label>
                            <div className="relative">
                                <input 
                                    type="text" 
                                    className="w-full bg-slate-800 text-white rounded-xl p-3 pl-10 focus:outline-none focus:ring-2 focus:ring-violet-500/50 border border-slate-700 focus:border-violet-500/50 transition-all placeholder:text-slate-600"
                                ...
                                {isSearching && (
                                    <span className="material-symbols-outlined absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 animate-spin">progress_activity</span>
                                )}
                            </div>

                            {/* Search Results */}
                            {!selectedUser && searchQuery && searchResults.length > 0 && (
                                <div className="mt-2 bg-slate-800 rounded-lg border border-slate-700 overflow-hidden max-h-48">
                            ...
                            {selectedUser && (
                                <div className="mt-2 p-3 bg-violet-600/10 border border-violet-500/20 rounded-xl flex items-center justify-between">
